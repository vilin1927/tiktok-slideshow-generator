    ...
    try:
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            if audio_duration > 0:
                ...
                logger.info(f"{log_prefix}Audio duration: {audio_duration:.1f}s, {slide_duration:.2f}s per slide")
            else:
                logger.warning(f"{log_prefix}Could not read audio duration, using {slide_duration}s per slide")
        else:
...
def create_videos_for_variations(
    ...
    # Sort images within each group by slide order
    # Expected order: hook, body_1, body_2, ..., product
    ...
    for var_key, images in sorted(variation_groups.items()):
        ...
        sorted_images = sorted(images, key=slide_sort_key)
